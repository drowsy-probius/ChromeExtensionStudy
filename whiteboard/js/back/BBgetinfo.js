...
let _MakePromiseArrayBB = (cids) => {
    ...
    for(let i = 0; i<cids.length; i++){
        ...
        let PromiseTmp = (_promiseURLGETnewRequest(newrequest, CAurl)
                            .then(function(responseText){ // resolve [{num, title, content, author}]
                                return _promiseGetCourseAnnouncements(responseText)  
                            })
                            .then(function(content){    // resolve course ids
                                return _promiseSetCourseData(content, cids[i]);
                            })
                            .then(function(cid){        // content id의 배열 리턴
                                return _promiseGetCourseContentIDs(cid);
                            })
                            .then(function(ids){        // [[result1], [result2]] 리턴
                                return Promise.all(ids.forEach(id => {
                                    return (_promiseGetCourseContents(newrequest, id)
                                            .then(function(content){
                                                return _promiseSetCourseData(content, id);
                                            })
                                        )
                                }))
                            })
...
let _promiseGetCourseAnnouncements = function(responseText){
    return new Promise((resolve, reject) => {
        let tmp = new HTMLElement(responseText);
        ...
        if (announcementList) {
            for (let i = 0; i < announcementList.children.length; i++) {
                ...
                if (title) {
                    if(!content) content = "";
                    if(!author) author = "";
                    content.push({"order":i, "title":title, "content":content, "author":author});
                }
            }
            resolve(content);
        }
        reject(Error("Cannot fetch announcements."));
    });
}

let _promiseSetCourseData = function(contentlist, id){
    return ( _promiseSearchID(id)
                ...
                .catch(function(e){
                    return new Promise((resolve, reject)=>{
                        courseData.push({"id":id, "content":contentlist});
                        resolve(id);
                    })
                })
...
let _promiseGetCourseContents = function(newrequest, id){
    let CCurl = "https://kulms.korea.ac.kr/webapps/blackboard/content/listContent.jsp?course_id=${courseid}&content_id="+id;

    return ( _promiseURLGETnewRequest(newrequest, CCurl)
                .then(function(responseText){
                    return new Promise((resolve, reject) => {
                        let tmp = new HTMLDocument(responseText);
                        ...
                        if (contents) {                
                            for (let i = 0; i < contents.children.length; i++) {
                                ...
                                let file = contents.children[i].children[2].children[0].innerText;
                                let content = contents.children[i].children[2].children[1].innerText;
                        ...
                        reject(Error("Cannot fetch course contents."));
                    })
                })
